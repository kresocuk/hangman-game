...
def main_menu():
    while True:
        ...
        logo()
        draw_text('Početni menu', font, (0, 0, 0), screen, 260, 50)

        mx, my = pygame.mouse.get_pos()

        button_1 = pygame.Rect(300, 120, 200, 50)
        button_2 = pygame.Rect(300, 200, 200, 50)
        draw_text('Započni igru', sfont, (30,144,255), screen, 315, 130)
        draw_text('Opcije igre', sfont, (30,144,255), screen, 325, 210)
        ...
        pygame.draw.rect(screen, (0, 0, 0), button_2, 4)
...
def game():
    ...
    def hangman():
        ...
        while play == True:
            ...
            text = pygame.font.Font("freesansbold.ttf", 40)
            textSurf = text.render("Izaberi kategoriju", True, black)
            textRect = textSurf.get_rect()
            textRect.center = ((width / 2), (height / 2))
            screen.blit(textSurf, (240, 50))

            button("Životinje", 300, 150, 200, 75, black, lightgrey, Animals)
            button("Automobili", 300, 240, 200, 75, black, lightgrey, Vehicles)
            button("Hrana", 300, 330, 200, 75, black, lightgrey, Foods)
    ...
    def hangmanGame(catagory, title):
        ...
        while gamePlay == True:
            ...
            textSurf = text.render(("Pokusaja: %s" % chances), False, black)
            textRect = textSurf.get_rect()
    ...
    def Animals():
        animal = ['macka', 'pas', 'krava', 'kokos', 'ovca', 'koza', 'sova', 'galeb', 'jastreb']
        print("animal")
        title = "Životinje"
        hangmanGame(animal, title)

    def Vehicles():
        vehicle = ['bmw', 'volkswagen', 'audi', 'ford', 'toyota', 'renault', 'opel', 'seat' ]
        print("vehicle")
        title = "Automobili"
        hangmanGame(vehicle, title)

    def Foods():
        food = ['jabuka', 'kruska', 'naranca', 'sljiva', 'banana', 'tresnja', 'visnja', 'luk', 'krastavac', 'tikva']
        print("food")
        title = "Hrana"
        hangmanGame(food, title)

    def Sports():
        sport = ['nogomet', 'kosarka', 'tenis', 'odbojka', 'plivanje', 'stolni tenis', 'trcanje']
        print("sport")
        title = "Sportovi"
...
def options():
    ...
    while running:
        ...
        draw_text('Opcije', font, (0, 0, 0), screen, 20, 20)
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
                sys.exit()
